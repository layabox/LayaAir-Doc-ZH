...
开启自动调整后,引擎会根据Spine文件中的原点(0,0)自动计算并设置节点锚点,使其与Spine编辑器中的原点对齐。这样开发者即可方便地基于锚点进行动画的镜像、旋转等操作。如动图2-7所示，需要注意,**启用该功能后,Spine组件所属节点的自身锚点设置将失效。**如需进一步偏移,可通过Spine的偏移属性或嵌套一层节点来实现。
