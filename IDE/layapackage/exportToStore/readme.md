...
另一篇文档《[包管理器与资源包导入](../pluginImport/readme.md)》面向资源包的使用者。
...
当上传的资源属于插件安装包时，资源包标识（即包名）必须按规范填写。包名的格式统一为 `com.商店名.包名`，例如 `com.layabox.spine-baking`。其中，包名部分通常是一个英文单词，如需多个单词组成，可以使用中划线连接。所有字母应全部采用小写形式，以确保规范性和统一性。
