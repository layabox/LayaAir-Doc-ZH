...
在前文提到，每次修改模型设置都会导致重新导入。如果开发者通过自定义IDE插件对模型频繁修改，可能会导致不断的调起模型导入处理工具，影响IDE的体验。

另外，除了属性面板上的基础设置外。如果想对模型内的动画、材质等进行修改，也是无法做到的。
...
提取模型的内部资源，不仅开发者可以自由修改模型内部的数据，不需要在每次保存时重新导入，对于动画文件中的一些格式错误，在提取的过程中，也会自动进行纠错处理。

## 二、模型的场景(Scene)导入设置说明

一个模型中，可能会包含很多信息，例如摄像机、光源等场景相关的信息，以及网格、动画、材质、骨骼等。我们对这些信息进行分类，首先从场景相关的设置开始介绍。

### 2.1 模型单位转换convert Units

模型导入进来之后的默认单位是**米**，可以通过此处设置改为**厘米**单位，或还原回**米**单位。

在单位转换过程中，为了避免模型的突然放大或缩小。默认的流程是保持外观大小，但是会做出缩放处理。
...
如果是厘米转换为米，则没有这个选项，会自动处理转换流程。被放大的尺寸会还原，没放大的尺寸保持不变。

### 2.2 模型缩放scale Factor

模型缩放主要用于根据目标场景尺度来预处理模型大小，通过模型的整体缩放来处理不同模型之间的大小比例，使其符合设计需求。

例如，上一小节`模型尺寸不变`的功能，本质上就是基于模型缩放处理实现的。
...
| 匹配规则            | 作用                                                         |
| ------------------- | ------------------------------------------------------------ |
| 材质名称            | 材质名称仅由**材质的文件名**组成                             |
| 模型名称 + 材质名称 | 材质名称的组成规则为：模型文件名称-材质文件名称，<br />例如 model_01-material_01，model_01为模型名称，material_01为材质名称 |
...
图中的两个选项比较容易理解，就是在当前的materials目录下查找，或者在当前以及上级的materials目录下查找。

如果找到与名称匹配规则相符的材质，则会自动进行材质映射。