...
在 `LayaAir3-IDE` 的默认构建发布流程中，项目会将 `src` 目录下的所有 `TypeScript` 脚本统一编译并打包为一个名为 `bundle.js` 的脚本集合文件。我们称之为主脚本集。
...
然后通过严格的目录检查机制确保**只有脚本集所在目录下的文件才会被最终包含到输出中**，

非该目录下的文件即使在`entries`中指定也会被过滤掉。这种机制既提供了精确的文件选择能力，又保证了脚本集的封装性和模块隔离性。
...
入口文件中的脚本，即使引用了不在入口文件`entries`中指定的其它脚本类，这些类也会被编译进 脚本集定义 对应的 模块化脚本集中。  

但若涉及 **跨脚本集依赖**（依赖另一个脚本集定义的脚本功能时），则需在**依赖** `references` 中显式声明，
...
如果**不勾选**，构建发布系统会检测脚本集中引用的文件是否属于其他脚本集，如果引用了其他脚本集的文件，不会将外部脚本文件内容编译打包进来，而是创建对该脚本集的引用。
