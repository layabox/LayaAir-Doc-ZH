---
title: "自定义物理引擎库"
description: "LayaAir引擎内置了第三方的物理引擎，例如，2D内置了Box2D，3D内置了Bullet和PhysX。"
slug: "3d/advanced/customphysicsengine"
---
...
- cannon.png是安装包的图标，需要注意的是，这个包图标必须要放到editorResources下才会能被安装包找到。这类似于assets资源目录下`resources`目录的作用。
- package.json 是安装包的关键配置文件，能否被识别为安装包，全靠它。
...
- engine 表示这是引擎模块的配置
- engine.name  是引擎模块下的哪个配置，值`laya.physics3D`是表示3D物理引擎的配置。
- engine.addons 表示在 laya.physics3D这个引擎内置的配置下，增加新的选项。
...
Cannon.js 插件地址：[https://store.layaair.com/info.php?id=10180](https://store.layaair.com/info.php?id=10180)
