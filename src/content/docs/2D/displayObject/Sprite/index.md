---
title: "精灵（Sprite）"
description: "游戏中Sprite精灵是在屏幕中能够被控制的显示对象，如果屏幕中的显示对象不能被控制那就只是一个节点。准确来说，Sprite精灵是一个能够通过改变自身属性，比如角度、位置、缩放、颜色等，变为可控动画的2D图像。"
slug: "2d/displayobject/sprite"
---
...
纹理 `Texture`：绘制一个图片或者渲染纹理。

绘制图形 `Graphics`：绘制一个或者一组图形。
...
> **注意：在实现项目开发过程中，如果只是绘制一张图片，那么尽量使用Spirte对象通过Texture的方式绘制，性能是最高的。在2D界面开发中，需要将图片编辑成“精灵纹理”，详细请参考[《项目资源面板说明》](/basics/ide/assets/)。**
...
具体每一种图形的绘制介绍和代码使用方法，请开发者参考[《绘制图形》](/ide/uieditor/graphics/)一篇。
...
| 其它属性                          | 功能说明                                                     |
| --------------------------------- | ------------------------------------------------------------ |
| 混合模式 **Blend Mode**           | 指定要使用的混合模式。可选 normal、add、multiply、screen、overlay、light、lighter、mask、destinationOut 共9种 |
| 接受鼠标时间 **Mouse Enabled**    | 是否接受鼠标事件                                             |
| 鼠标穿透 **Mouse Through**        | 鼠标事件与此对象的碰撞检测是否可穿透                         |
| 优先本对象 **Hit Test Prior**     | 指定鼠标事件检测是优先检测自身，还是优先检测其子对象         |
| Z索引 **Zindex**                  | 控制 2D 场景中物体或元素的渲染层级和堆叠顺序                 |
| 堆叠根节点                        | 当一个节点成为堆叠根节点后，它所有子节点的 `zIndex` 都是**相对于它自己**来计算的，而不是相对于整个场景的根节点，默认为false |
| 优化DrawCall **DrawCallOptimize** | 是否开启DrawCall优化                                         |
| 缓存模式 **Cache As**             | 是否开启静态缓存优化                                         |
| 遮罩 **Mask**                     | 设置遮罩节点对象                                             |
| 点击区域 **Hit Area**             | 点击区域                                                     |
| 后期处理 **Filters**              | UI滤镜效果                                                   |
...
> **注意：IDE中共提供normal、add、multiply、screen、overlay、light、lighter、mask、destinationOut九种模式。其中lighter模式，原有图形和新图形都显示，交集部分做颜色叠加**



#### 2.4.3 设置Draw Call Optimize
...
在实际应用中，特别是UI界面，节点树通常很复杂，会存在大量不同图集的图片交错，或者文字和图片交错的情况出现。这时，动态合批发挥作用的空间就会变小，降低DC的效果不明显。如果要求开发者搭建界面时注意图片和文字分层，手工将相同图集的图片安排到连续的顺序，这会增加开发者负担，影响UI的结构化，制作和维护都非常困难。

开启DrawCall优化这个属性解决了这个问题，勾选此属性后，引擎可以在保持原UI结构的前提下，利用动态合批的特性，我们来看一个例子：
...
下面是DrawCall优化使用上的一些最佳实践：

1） 如果节点数量太多可能会消耗一定CPU。假如一个Scene里有两个子界面，每个界面有200个子节点，那么更建议每个界面分别打开DrawCall优化，而不是直接在Scene节点上打开DrawCall优化。

2）物体移动，重绘等会引发重新合批，所以静态界面要比动态界面合批效率高。如果一个复杂的动态界面，优化DC的收益不高（比如只少了几个DC），那打开优化功能是不划算的。

3）在编辑器预览时，因为自动图集还是以散图的形式加载，所以这些图片无法合批。这种情况下需要在发布后才能看到真实的效果。自动图集预览时生成图集的功能后续版本会推出。

4）一般情况下，界面上的文本都使用同一个材质，但当文本过多时， 一张2048贴图放不下后，会造成文本在不同贴图的情况，所以文本有时会出现无法合并的情况。
...
`cacheAs`：缓存组件，是否缓存为静态图像，合理作用能提高性能 。它有”无”和”位图”两个值可选。

**“无”**：表示不做任何缓存。

> 旧版本中的”正常”（`normal`，命令缓存）已在LayaAir 3.4中移除，代码里赋值为`"normal"`时等同于`"none"`，即不做任何缓存。
...
**Tips**：*当cacheAs选择”位图”时，子对象发生变化，会自动重新缓存，同时也可以手动调用reCache方法更新缓存。*
...
| **属性**                      | **功能说明**                                                 |
| :---------------------------- | :----------------------------------------------------------- |
| 接收鼠标事件 **MouseEnabled** | 设为true表示接受鼠标事件，设为false不接受鼠标事件（以下属性均将失效） |
| 点击区域 **Hit Area**         | 点击区域，自定义区域只支持圆形、矩形、多边形                 |
| 鼠标穿透 **MouseThrough**     | 默认值为false，如果设置为true，则点击空白区域（不设置Texture、Image等）可以穿透过去，只针对自身有效 |
| 优先本对象 **Hit Test Piror** | 当值为true时，并且不穿透（Mouse Through为false）时，会优先检测本节点。当值为false时，会优先检测子节点，一直到stage上的全部节点都递归检测完毕。 |
...
> 事件监听方法参考[《事件与交互》](/basics/common/event/)。
...
后期处理主要是用来实现图像的各种特殊效果，使图像取得最佳艺术效果。后期处理的类型有很多，但是营造不同的效果需要不同的后期处理功能。
...
通过此属性来调整渲染顺序是作用于全局的，不受节点树的限制，也不会印象节点在节点树的逻辑顺序。
...
如果设置C的zIndex为1，则渲染顺序将变成：

S->A->D->B->E->F->C

再设置D的zIndex为-1，则渲染顺序将变成：

D->S->A->B->E->F->C

zIndex的值越大，渲染顺序越靠后，显示层级越靠上。



注意zIndex的数值是一个相对父节点的数值，例设父节点的zIndex是1，子节点的zIndex是2，那么这个子节点最终的有效zIndex值是3。例如，设置A的zIndex为1，C的zIndex为1，B的zIndex为1，则渲染顺序将变成：

S->D->E->F->A->B->C 

C在最后渲染，因为C的有效zIndex值是2。



在实际应用中，通常需要在局部调整节点的渲染顺序，而非全局。例如在一个预制体内如果希望某个节点显示在最上面，所以设置了zIndex值为一个正数，但当这个预制体加入场景后，如果其它节点没有设置zIndex属性，那这个节点就会显示在所有节点的上层，而不是只在这个预制体内容的最上层，这显然不符合预期。
...
还是以上面的例子说明，如果设置A的stackingRoot为true，设置C的zIndex为2，D的zIndex是1，则渲染顺序将变成：

S->A->D->C->B->E->F 

可以看到，虽然C和D的zIndex都比BEF大，但他们不会显示在BEF上。

这时，如果设置A的zIndex为3，则渲染顺序将变成：

S->B->E->F->A->D->C

注意这里是ADC而不是DCA，也就是说，设置为stackingRoot的节点，它自身的zIndex表达的仍然是全局的渲染顺序。



Sprite上还有一个属性zOrder，这个属性会改变子节点在父节点中的逻辑顺序，同时也会改变渲染顺序；

这个属性没有暴露在IDE中，只能通过代码来设置：
