...
| 其它属性                          | 功能说明                                                     |
| --------------------------------- | ------------------------------------------------------------ |
| 混合模式 **Blend Mode**           | 指定要使用的混合模式。目前只支持"lighter"                    |
...
| 堆叠根节点                        | 当一个节点成为堆叠根节点后，它所有子节点的 `zOrder` 都是**相对于它自己**来计算的，而不是相对于整个场景的根节点，默认为false |
...
Sprite上还有一个属性zOrder，这个属性会改变子节点在父节点中的逻辑顺序，但并不改变渲染顺序；
