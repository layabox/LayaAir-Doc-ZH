---
title: "音频节点(SoundNode)"
description: "如图1-1所示，可以在层级窗口中右键进行创建，也可以从小部件窗口中拖拽添加。"
slug: "2d/displayobject/soundnode"
---
...
| ------------------------ | ------------------------------------------------------------ |
| 源 **Source**            | 添加音频文件源                                               |
| 是否背景音乐 **IsMusic** | 是否为背景音乐。勾选后，当前音频为背景音乐，如果有两个音频都勾选了IsMusic，则不能同时播放，只能播放一个。如果不勾选，当前音频文件可以与背景音乐、其它音频同时播放 |
| 循环次数 **Loop**        | 循环播放的次数。设置为0，是无尽循环；设置为1，只播放一次     |
| 自动播放 **AutoPlay**    | 运行时，是否自动播放                                         |
