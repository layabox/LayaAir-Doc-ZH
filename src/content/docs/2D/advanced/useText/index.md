---
title: "文本进阶使用"
description: "LayaAir引擎与IDE提供了丰富的文本组件，适用于各种文本应用场景。本篇分别就IDE中的显示文本组件、位图文本组件、可输入文本组件，这几个类型的组件进行介绍，让大家全面了解一下LayaAir引擎的各个文本组件。"
slug: "2d/advanced/usetext"
---
...
![1-1](/2D/advanced/useText/img/1-1.png)

（图1-1） 

值为`60px simHei italic` 增加了斜体效果。如下图1-2所示：

![1-2](/2D/advanced/useText/img/1-2.png)

（图1-2）

值为`bold 60px simHei italic` 增加了文本加粗效果。如下图1-3所示：

![1-3](/2D/advanced/useText/img/1-3.png)
...
![2-1](/2D/advanced/useText/img/2-1.png)
...
![2-2](/2D/advanced/useText/img/2-2.png)
...
![2-3](/2D/advanced/useText/img/2-3.png)

（图2-3）

创建后，如图2-4所示，会生成一个`BitmapFont.fnt`字体文件，

![2-4](/2D/advanced/useText/img/2-4.png)

（图2-4）

点击选中后，在如图2-5所示的属性设置面板中，就可以创建位图字体了。

![2-5](/2D/advanced/useText/img/2-5.png)

（图2-5）

点击“编辑字符集”的`+`创建一个字符，如图2-6所示：

![2-6](/2D/advanced/useText/img/2-6.png)
...
![2-7](/2D/advanced/useText/img/2-7.png)
...
![2-8](/2D/advanced/useText/img/2-8.gif)
...
在所有的文本相关的UI组件上使用HTML标签，并且可以与其它的UBB标签、文本模板、普通文本混合使用，开发者只需设置对应的syntax属性即可，支持的语法与使用方式请参考文档[《基础文本》](/2d/displayobject/text/)。
