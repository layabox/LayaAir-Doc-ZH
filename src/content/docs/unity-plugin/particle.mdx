...
![导出窗口的粒子导出默认模式开关](./img/particle-export-mode.png)

**第 3 级 · 全局默认：** 导出窗口「其他设置」里的 `Particle Export Mode` 开关（`Shuriken (GPU)` / `CPU Particle`）——作用于没挂覆盖组件、也未命中自动检测的粒子。

![LayaParticleExportSetting 组件](./img/particle-export-setting.png)
...
![Unity 粒子系统检视器](./img/particle-system-inspector.png)
...
![Unity 中的 GPU 粒子](./img/particle-gpu-unity.png)

**Unity：** 默认粒子系统（Billboard，柔和圆形粒子）。

![LayaAir 中的 GPU 粒子](./img/particle-gpu-laya.png)
...
![Unity 中的 CPU 粒子](./img/particle-cpu-unity.png)

**Unity：** 含 Noise 模块的粒子，命中自动检测 → CPU 路径。

![LayaAir 中的 CPU 粒子](./img/particle-cpu-laya.png)
...
![Unity 原版运行效果](/unity-plugin/img/vfx-result-unity.jpg)

![LayaAir 中转换后运行效果（贴图 / shader 补齐后）](/unity-plugin/img/vfx-result-laya.jpg)
