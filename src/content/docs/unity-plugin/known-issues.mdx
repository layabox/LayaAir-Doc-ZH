---
title: "疑似源码问题清单"
description: "文档编写过程中基于插件 1.1.7 源码核查与实测发现的疑似问题记录：Help 菜单链接、只读包纹理崩溃、自定义 Shader 转换、BlendTree、粒子材质混合因子与渲染对齐等十四条，每条含现象、根因与修复方向。"
slug: "unity-plugin/known-issues"
---

:::caution
本清单为文档编写过程中发现的**疑似源码问题**，未直接改源码，记录在此交插件作者决定。每条包含：文件 + 现象 + 根因 + 修复方向 + 状态。内容基于插件 **1.1.7** 版本，后续版本可能已修复。
:::
...
**状态**：3 种失败形态已坐实，见[自定义Shader导出](/unity-plugin/custom-shader/)。
...
**根因**：相机是 Game Kit 的 `KeyboardAndMouseFreeLookRig` 脚本驱动；脚本未配 UUID 映射 → 不导出 → Laya 里相机只剩静态 transform、无运行时 rig 驱动。属脚本逻辑能力边界（已在[能力一览](/unity-plugin/)声明）。
...
**状态**：源码确认；[2D精灵导出](/unity-plugin/sprite-2d/)已加边界说明。
...
**状态**：源码确认；[材质与Shader导出](/unity-plugin/material-shader/)用中性表述。
...
| Unity 正常 | Laya 塌环 |
| --- | --- |
| ![HitParticle 在 Unity 中的正常效果](/unity-plugin/issue-shots/issue-13-hitparticle-unity.png) | ![HitParticle 在 Laya 中塌陷成环](/unity-plugin/issue-shots/issue-13-hitparticle-laya.png) |
...
| --- | --- |
| ![AcidSplash 在 Unity 中平躺贴地](/unity-plugin/issue-shots/issue-14-acidsplash-unity.png) | ![AcidSplash 在 Laya 中立起](/unity-plugin/issue-shots/issue-14-acidsplash-laya.png) |
