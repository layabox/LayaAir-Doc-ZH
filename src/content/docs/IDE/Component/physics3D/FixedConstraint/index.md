---
title: "固定约束 FixedConstraint"
description: "固定约束（FixedConstraint）将两个物体完全固定在一起，限制所有的相对平移和旋转运动，就像被焊接在一起一样作为一个整体运动。常用于将武器固定到角色手上、将零件固定到机器上等场景。"
slug: "ide/component/physics3d/fixedconstraint"
---
...
默认值均为极大值（`Number.MAX_VALUE`），表示约束不会断裂。如果需要模拟可破坏的连接（如门被撞开、零件脱落），可以将它们设为合理的数值。
