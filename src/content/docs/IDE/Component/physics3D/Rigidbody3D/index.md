...
### 1.11 CCD连续碰撞检测阈值 ccdMotionThreshold

用于连续碰撞检测（CCD）的速度阈值。当物体移动速度超过该值时，物理引擎会启用连续碰撞检测，防止快速移动的物体（例如子弹）穿透其他物体。值为0表示禁用CCD。

### 1.12 CCD扫描球半径 ccdSweptSphereRadius

获取或设置用于连续碰撞检测（CCD）的扫描球半径。该半径定义了CCD检测时使用的球体大小，配合ccdMotionThreshold一起使用以实现精确的连续碰撞检测。
