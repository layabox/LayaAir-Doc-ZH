---
title: "3D刚体"
description: "在LayaAir3引擎中，3D刚体的类为 Rigidbody3D，继承自 PhysicsColliderComponent。本文档介绍碰撞器基类的通用属性、刚体的专有属性，以及力的应用方法。"
slug: "ide/component/physics3d/rigidbody3d"
---

> Author : Charley

在LayaAir3引擎中，3D刚体的类为 **Rigidbody3D**，继承自 PhysicsColliderComponent。本文档介绍碰撞器基类的通用属性、刚体的专有属性，以及力的应用方法。
...
组合碰撞形状 是将多个子碰撞形状（称为子形状）通过相对于父节点的局部偏移组合成一个整体的碰撞体。其本质是在物理引擎底层创建一个由多个形状组成的“组合体”，作为一个刚体（RigidBody）处理。
...
xxx.canCollideWith = 1 << 2;  //只与分组ID为2的（值为4）分组发生碰撞
...
### 1.7 动态摩擦力 dynamicFriction

动态摩擦力是指两个物体在相对运动时产生的摩擦力。与静态摩擦力不同，动态摩擦力作用于已经在运动中的物体。值越大，运动中的物体越难以继续滑动。

### 1.8 静态摩擦力 staticFriction

静态摩擦力是指两个静止接触的物体之间的摩擦力，用于阻止物体从静止状态开始运动。值越大，物体越难以开始滑动。

### 1.9 摩擦力组合模式 frictionCombine

摩擦力组合模式用于定义两个碰撞体之间的摩擦力如何组合计算。当两个物体接触时，需要确定最终的摩擦系数，该属性决定了组合方式。

### 1.10 弹力组合模式 restitutionCombine

弹力组合模式用于定义两个碰撞体之间的恢复系数如何组合计算。与摩擦力组合模式类似，该属性决定了碰撞双方的弹力的组合计算方式。

### 1.11 CCD连续碰撞检测阈值 PhysicsSettings.ccdThreshold

> 注意：CCD（连续碰撞检测）不是碰撞器组件上的属性，而是全局物理设置 `PhysicsSettings` 中的配置项，需通过 `PhysicsSettings.enableCCD` 开启（默认为 `false`），对整个物理世界生效。

用于连续碰撞检测（CCD）的速度阈值。当物体移动速度超过该值时，物理引擎会启用连续碰撞检测，防止快速移动的物体（例如子弹）穿透其他物体。

### 1.12 CCD扫描球半径 PhysicsSettings.ccdSphereRadius

同样属于全局物理设置 `PhysicsSettings`。用于连续碰撞检测（CCD）的扫描球半径，该半径定义了CCD检测时使用的球体大小，配合 `PhysicsSettings.ccdThreshold` 一起使用以实现精确的连续碰撞检测。
...
默认的重力值为 **(0, -10, 0)**，表示物体会以 10 m/s² 的加速度向下坠落。
...
### 2.11 碰撞检测模式 collisionDetectionMode

碰撞检测模式用于设置刚体的碰撞检测方式。默认的离散碰撞检测对于大部分场景足够，但当物体移动速度非常快时，可能会出现穿透现象。此时可以切换为连续碰撞检测模式来提高检测精度。

### 2.12 允许休眠 allowSleep

允许休眠属性用于设置刚体是否可以进入休眠状态。当刚体的速度低于休眠阈值时，物理引擎会将其标记为休眠状态以节省计算资源。休眠的刚体不会参与物理模拟，直到被外力唤醒。默认开启。

### 2.13 休眠阈值 sleepThreshold

刚体睡眠的线速度阈值。当刚体的线速度低于该阈值时，刚体可能进入休眠状态。值越大，刚体越容易进入休眠。

### 2.14 休眠角速度阈值 sleepAngularThreshold

刚体睡眠的角速度阈值。当刚体的角速度低于该阈值时，配合线速度阈值共同判断刚体是否可以进入休眠状态。

### 2.15 是否正在休眠 isSleeping

该属性用于获取刚体当前是否处于休眠状态。这是一个只读属性，返回布尔值。

### 2.16 唤醒刚体 wakeUp()

该方法用于手动唤醒处于休眠状态的刚体，使其重新参与物理模拟。
...
在代码中，可以通过以下方法对刚体施加力和冲量，实现物理驱动的运动效果。

### 3.1 施加力 applyForce()

用于向刚体施加一个持续的力。力会根据刚体的质量产生加速度，持续施加力才能保持加速效果。可以指定力的方向和大小，以及力的施加点（本地坐标偏移）。

代码设置示例如下：

```typescript
//向刚体施加一个向上的力
rigidbody3D.applyForce(new Laya.Vector3(0, 10, 0));

//向刚体的指定位置施加力（本地坐标偏移）
rigidbody3D.applyForce(new Laya.Vector3(0, 10, 0), new Laya.Vector3(1, 0, 0));
```

### 3.2 施加冲量 applyImpulse()

用于向刚体施加一个瞬间冲量。与持续力不同，冲量会立即改变刚体的速度。适用于模拟爆炸冲击、弹射等瞬间效果。

代码设置示例如下：

```typescript
//向刚体施加一个向前的冲量
rigidbody3D.applyImpulse(new Laya.Vector3(0, 0, 5));

//向刚体的指定位置施加冲量
rigidbody3D.applyImpulse(new Laya.Vector3(0, 0, 5), new Laya.Vector3(1, 0, 0));
```

### 3.3 施加扭转力 applyTorque()
...
```typescript
//向刚体施加绕Y轴的扭转力
rigidbody3D.applyTorque(new Laya.Vector3(0, 10, 0));
```

### 3.4 施加扭转冲量 applyTorqueImpulse()

用于向刚体施加一个瞬间扭转冲量，立即改变刚体的角速度。

代码设置示例如下：

```typescript
//向刚体施加绕Y轴的扭转冲量
rigidbody3D.applyTorqueImpulse(new Laya.Vector3(0, 5, 0));
```
