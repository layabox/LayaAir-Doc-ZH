---
title: "3D寻路"
description: "在游戏开发中，3D寻路是一个关键的功能，特别是在需要模拟角色或物体在三维空间内自动导航和移动的游戏中。3D寻路使得游戏内的角色能够智能地在复杂的三维环境中找到从一个点到另一个点的最优路径，同时避免障碍物和其他潜在障碍。"
slug: "ide/component/navmesh"
---
...
### 2.2 动态节点的基类(BaseNav3DModifle)

有时，场景中需要一些动态节点，如移动的地板、临时添加的障碍物等。这些就需要导航网格随着节点的变化而变化，`动态导航表面`和`导航障碍物`两个组件可以实现此功能。它们都继承自动态节点基类BaseNav3DModifle，它的数据可以动态添加到NavMeshSurface类的节点下面，形成场景路径的动态变化。下面分别对其两个子类进行介绍。
