---
title: "树状列表组件（Tree）"
description: "Tree 组件用来显示树状结构。用户可以查看排列为可扩展树的层次结构数据。Tree 组件的脚本接口请参考 Tree API。"
slug: "ide/uieditor/uicomponent/tree"
---
...
- 单元格选择状态切片动画 Clip 资源（clip_selectBox.png）：
  ...
  切片数为2，切片索引从0开始依次代表：单元格悬停状态图（白色）、单元格选中状态图。
...
| 子项模板 **itemTemplate**    | 渲染单元，具体用法已在1.2节的步骤中给出。                    |
| 滚动条皮肤 **scrollBarSkin** | 滚动条皮肤。                                                 |
| 选中项索引 **selectedIndex** | 当前选择的项索引。-1表示初始状态下无选取项。                 |
| 子项间距  **spaceBottom**    | 每一项之间的间隔距离。单位是像素。                           |
| 子项缩进距离 **spaceLeft**   | 子项的左侧缩进距离。单位是像素。                             |
