...
| 自适应大小 **fitContent**      | 否（"no"）：无操作；高度和宽度（"yes"）：Label边框可以根据文字大小和数量，自动调整高度和宽度为合适大小；高度（"height"）：Label边框只自动调整高度 |
...
| 子项对齐 **alignItems**        | 子项对齐，图文混排时图片和文字的对齐方式，分别是top（居顶对齐）、middle（居中对齐）、bottom（居底对齐） |
