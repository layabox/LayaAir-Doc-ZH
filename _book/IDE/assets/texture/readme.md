...
面对`2D`开发需求时，也可以用于`2D` Shader以及支持设置纹理偏移的`2D`渲染组件（例如`2D`线渲染器）中。
...
**纹理旋转** 用于改变纹理在表面上的朝向，以 90° 为单位进行顺时针（左）或逆时针（右）调整。
...
#### 4.4.4 多级纹理淡化 `fadeOutEnable`

多级纹理淡化是一种用于 **平滑 Mipmap 级别切换时间点（距离）** 的技术。旨在消除当物体远离或靠近摄像机时，纹理突然变模糊或变清晰的视觉突变问题。

在前文中，我们介绍过多级纹理与三线性过滤结合来解决突变问题，但多级纹理淡化解决突变的方案与之前有所不同。

即使使用了三线性过滤，纹理的锐度是平滑过渡的，但 Mipmap 级别切换的**“时机”**（即**距离**）仍然是突然的。

例如：物体在距离 `50` 米时，系统判断应该用级别4的纹理。在 `50.001` 米时，系统判断应该用级别5的纹理。

虽然三线性过滤确保了 Level 4 和 Level 5 之间颜色是平滑的，但 Level 5 相比 Level 4 已经**丢失了更多的高频细节**（更模糊）。当物体穿过 `50` 米这个界限时，人眼仍然会察觉到**细节的突然缺失**（整个物体的纹理突然在**一帧之内**变模糊了）。这种现象称为**时机跳变**。

多级纹理淡化技术通过在 `48` 米到 `52` 米之间（一个过渡范围），**动态调整不同级别纹理偏移来实现平滑过渡**。它不是瞬间切换，而是逐渐地增加这个偏移量，使得纹理的**锐度损失是渐进式的**，从而在视觉上消除了这种时机跳变。

所以，**三线性过滤** 解决了多级纹理之间 **空间上** （颜色）的不连续性。而**多级纹理淡化** 解决了切换 **时机或距离上** 的不连续性。他们结合使用会进一步提升视觉流畅度。

#### 4.4.5 淡化设置`fadeOutMipmap`

在勾选多级纹理淡化后，会出现一个关联属性 淡化设置，这个设置有两个值，S与E。分别代表淡化过程的**起始 Mipmap 级别（Start）和 结束 Mipmap 级别（End）**。

例如，S为3，E为6，会处理于3 → 4，4 → 5，5 → 6这些纹理级别之间的过渡效果。

通常，不处理远处模糊的级别纹理，因为远处纹理已经模糊，淡化效果几乎无法察觉，但计算资源却被浪费了。

如果过近，例如0 → 1级别差异较小，人眼通常难以察觉跳变，淡化处理显得多余。

所以，中段淡化处理更为常见。

#### 4.4.6 各向异性过滤等级`anisoLevel`
